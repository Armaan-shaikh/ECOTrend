...
import { NoiseQualityScoreResponse } from '../lib/types';
import { Volume2, ShieldCheck, Database, FileText } from 'lucide-react';

interface NoiseScoreCardProps {
  noiseScore: NoiseQualityScoreResponse | null;
  loading: boolean;
  onOpenMethodology: () => void;
}

export const NoiseScoreCard: React.FC<NoiseScoreCardProps> = ({
  noiseScore,
  loading,
  onOpenMethodology,
}) => {
  if (loading || !noiseScore) {
    return (
      <div className="bg-eco-card border border-eco-border rounded-2xl p-6 h-56 flex items-center justify-center animate-pulse">
        <div className="text-eco-muted text-sm font-semibold">Calculating Acoustic Disturbance Index...</div>
      </div>
    );
  }
  ...
  const strokeDashoffset = circumference - (overall_noise_score / 100) * circumference;
  ...
  return (
    <div className="bg-eco-card border border-eco-border rounded-2xl p-6 shadow-xl flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden">
      ...
      <div className="flex items-center gap-6">
        <div className="relative w-36 h-36 flex items-center justify-center flex-shrink-0">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
            <span className="text-3xl font-black text-eco-text font-mono">{overall_noise_score}</span>
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h2 className="text-lg font-bold text-eco-text">Acoustic Disturbance Index</h2>
          </div>
      ...
      {/* Right Column: Coverage & Methodology */}
      <div className="flex flex-col items-start md:items-end justify-between gap-3 w-full md:w-auto border-t md:border-t-0 border-eco-border pt-4 md:pt-0">
        ...
        >
          ...
          <span>EcoTrend Acoustic Methodology v1.0</span>
        </button>
      </div>
    </div>
  );
