...
import { X, BookOpen, ShieldAlert, FileText, Droplet } from 'lucide-react';
import { StandardsInfoResponse } from '../lib/types';
import { fetchWaterStandardsInfo } from '../lib/api';

interface WaterStandardsModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export const WaterStandardsModal: React.FC<WaterStandardsModalProps> = ({
  isOpen,
  onClose,
}) => {
  ...
  const loadInfo = async () => {
    const info = await fetchWaterStandardsInfo();
    setData(info);
  };
      ...
      <div className="w-full max-w-xl bg-eco-card border-l border-eco-border h-full p-6 flex flex-col shadow-2xl animate-in slide-in-from-right duration-300">
        <div className="flex items-center justify-between border-b border-eco-border pb-4 mb-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-base font-bold text-eco-text">Water Methodology & Standards</h2>
              <p className="text-xs text-eco-muted">EcoTrend Water Quality Health Scoring v1.0</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto space-y-5 pr-1">
          {/* Attribution Box */}
          <div className="bg-cyan-500/10 border border-cyan-500/30 rounded-2xl p-4 text-xs space-y-1.5">
            ...
            <p className="text-eco-text leading-relaxed">
              Official reference limits are sourced from <strong>WHO Guidelines for Drinking-Water Quality (4th ed.)</strong>, <strong>US EPA Secondary Drinking Water Standards</strong>, and <strong>USGS Ecological Limits</strong>.
            </p>
            <p className="text-eco-muted">
              The 0–100 normalization curves and weighting scheme represent <strong>EcoTrend's project-defined water methodology</strong>. Water Quality Score is kept strictly separate from Air Quality EHS.
            </p>
          </div>
          ...
          <div>
            <h3 className="text-xs font-bold uppercase tracking-wider text-eco-muted mb-2 flex items-center gap-1.5">
              ...
              Water Quality Metrics & Weight Rationale
            </h3>

            <div className="space-y-2.5">
              {data &&
                Object.values(data.standards).map((st: any) => (
                  <div key={st.metric} className="p-3.5 rounded-xl bg-eco-bg border border-eco-border text-xs flex flex-col gap-1.5">
                    ...
                    <div className="text-[11px] text-eco-muted font-mono bg-eco-card p-2 rounded flex items-center justify-between">
                      <span>Standard Reference: {st.standard_reference}</span>
                    </div>
          ...
          <div>
            <h3 className="text-xs font-bold uppercase tracking-wider text-eco-muted mb-2">
              Water Quality Score Categories
            </h3>
        ...
        <div className="border-t border-eco-border pt-4 mt-4 text-xs text-eco-muted flex items-center justify-between">
          <span>EcoTrend Water Methodology v1.0</span>
          <span className="text-eco-text font-mono">Freshwater Intelligence</span>
        </div>
      </div>
    </div>
  );
