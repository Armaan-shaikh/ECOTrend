...
interface SoilSubScoreCardsProps {
  subscores: MetricSubScore[];
}

export const SoilSubScoreCards: React.FC<SoilSubScoreCardsProps> = ({ subscores }) => {
  const getReferenceBadge = (refType?: string) => {
    if (refType === 'TOXICOLOGICAL_SCREENING') {
      return (
        <span className="flex items-center gap-1 text-[10px] font-semibold text-rose-400 bg-rose-500/10 px-2 py-0.5 rounded-full border border-rose-500/20" title="US EPA Ecological Soil Screening Level (Eco-SSL)">
          <ShieldCheck className="w-3 h-3" />
          <span>Screening</span>
        </span>
      );
    }
    if (refType === 'REGULATORY_LIMIT') {
      return (
        <span className="flex items-center gap-1 text-[10px] font-semibold text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded-full border border-amber-500/20" title="EU Directive 86/278/EEC / EPA Regulatory Limit">
    ...
    return (
      <span className="flex items-center gap-1 text-[10px] font-semibold text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-500/20" title="FAO-ISRIC Agronomic Guidelines">
        ...
        return (
          ...
          >
            <div className="flex items-start justify-between gap-2 mb-2">
              ...
              {isAvail ? getReferenceBadge(sub.reference_type) : <span className="text-[10px] font-bold text-rose-400 bg-rose-500/10 px-2 py-0.5 rounded-full">N/A</span>}
            </div>
