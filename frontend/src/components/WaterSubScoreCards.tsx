...
interface WaterSubScoreCardsProps {
  subscores: MetricSubScore[];
}

export const WaterSubScoreCards: React.FC<WaterSubScoreCardsProps> = ({ subscores }) => {
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-bold uppercase tracking-wider text-eco-muted flex items-center gap-1.5">
          <Droplet className="w-4 h-4 text-eco-cyan" />
          Water Metric Sub-Score Breakdown
        </h3>
        <span className="text-xs text-eco-muted font-mono">
          WHO 4th Ed / EPA Secondary Limits
        </span>
      </div>
