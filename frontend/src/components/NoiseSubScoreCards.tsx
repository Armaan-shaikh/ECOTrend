...
import { ShieldCheck, VolumeX, AlertTriangle } from 'lucide-react';

interface NoiseSubScoreCardsProps {
  subscores: MetricSubScore[];
}

export const NoiseSubScoreCards: React.FC<NoiseSubScoreCardsProps> = ({ subscores }) => {
  return (
        ...
        return (
          <div
            ...
            className={`bg-eco-card border border-eco-border rounded-2xl p-4 flex flex-col justify-between shadow-md transition ${
              !isAvail ? 'opacity-60 bg-eco-bg/30 border-dashed' : 'hover:border-eco-cyan/40'
            }`}
          >
            ...
            <div className="flex items-center justify-between text-[11px] font-bold border-t border-eco-border/60 pt-2 mt-1">
              ...
              <span className={`font-mono ${isAvail ? 'text-eco-cyan' : 'text-amber-400'}`}>
                {isAvail ? `${sub.score}/100` : 'UNAVAILABLE'}
              </span>
            </div>
          </div>
        );
